...
def issueBook():
    ...
    headingLabel = Label( headingFrame, text="Issue Book", bg='saddlebrown', fg='white', font=('New Roman', 13) )
    headingLabel.place( relx=0, rely=0, relwidth=1, relheight=1 )

    # Code
    lb1 = Label( top, text="Code : ", bg='brown', fg='white' )
    lb1.place( relx=0.075, rely=0.3, relheight=0.05 )

    code = Entry( top )
    code.place( relx=0.3, rely=0.3, relwidth=0.6, relheight=0.05 )

    # Book ID
    lb1 = Label( top, text="Book ID : ", bg='brown', fg='white' )
    lb1.place( relx=0.075, rely=0.4, relheight=0.05 )

    id = Entry( top )
    id.place( relx=0.3, rely=0.4, relwidth=0.6, relheight=0.05 )

    button = Button( top, text="SUBMIT", bg="black", fg="black", command=issueBookToStudent )
...
def issueBookToStudent():
    ...
    if (issue_book(issueCode,bookId)):
        headingLabel = Label( topForMessage, text="Book Issues", bg='white', fg='black', font=('New Roman', 13) )
        headingLabel.place( relx=0, rely=0, relwidth=1, relheight=1 )

    else:
        headingLabel = Label( topForMessage, text="Could not issue Book", bg='white', fg='black', font=('New Roman', 13) )
        headingLabel.place( relx=0, rely=0, relwidth=1, relheight=1 )
    top.destroy()
